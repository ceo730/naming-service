...
def _calc_jeolgi_day(year, solar_month, c_21, c_20, jeolgi_index=None):
    """寿星공식으로 절기 날짜 계산 (해당 월의 일)
    소한(11)/입춘(0) 등 1~2월 절기는 L=(Y-1)/4 사용 (寿星万年历 표준)
    """
    y = year % 100
    c = c_21 if year >= 2000 else c_20
    # 소한(index 11), 입춘(index 0): L = int((Y-1)/4) 사용
    if jeolgi_index is not None and jeolgi_index in (0, 11):
        L = int((y - 1) / 4)
    else:
        L = int(y / 4)
    day = int(c + 0.2422 * y - L)
    return day


# 절기 보정 테이블 (寿星공식 오차 보정, 실측 천문력 기반)
# L=(Y-1)/4 적용 후 남은 예외만 보정
_JEOLGI_OVERRIDES = {}
...
def get_jeolgi_date(year, jeolgi_index):
    ...
    day = _calc_jeolgi_day(year, solar_month, c_21, c_20, jeolgi_index)
